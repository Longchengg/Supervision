...
+ (NSArray *)showStringPickerDataSourceStyle:(NSInteger)style
{
    ...
    switch (style) {
        ...
        case 14:
        {
            NSArray *heightAry =[CGXStringPickerView yearArray:[dataSourceaa objectForKey:@"year"]];
            dataSource = heightAry;
...
//身高范围
+ (NSArray *)yearArray:(NSArray *)arr
{
    NSInteger inter1 = [arr[0] integerValue];
    NSInteger inter2 = [arr[1] integerValue];
    NSMutableArray *yearArr = [NSMutableArray array];
//    [yearArr addObject:@"不限"];
    for (NSInteger i = inter1; i<inter2; i++) {
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(nullable UIView *)view {
    ...
    if (!pickerLabel)
    {
        pickerLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 40)];
