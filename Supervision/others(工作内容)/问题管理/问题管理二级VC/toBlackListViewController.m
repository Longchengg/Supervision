...
@interface toBlackListViewController ()
<UITableViewDelegate,
...
@implementation toBlackListViewController
...
- (BKMenuListView *)menuListView{
    if (!_menuListView) {
        ...
        NSArray *arr = @[@"资质证书",@"安全生产许可证",@"四库一平台",@"锁",@"数字证书"];
        
        _menuListView = [[BKMenuListView alloc] initWithListViewType:BKMenuListViewTypeScrollView dataSourceArray:arr frame:CGRectMake(0, 0, SCREEN_WIDTH*2, 50) delegate:self];
        ...
        [_menuListView setSliderViewHeight:2 color:MColor widthValue:1 andIsYuanJiao:YES];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (0 == _selectedIndex) {
        ...
        cell.Btype =index;
        _countText = [NSString stringWithFormat:@"%@条待处理",_dataSourceArr[index][@"count"]];
        if ([_countText  isEqual: @"0条待处理"]) {
            _countText = @"";
        }
        cell.Rlabel = _countText;
        return cell;
        ...
        cell.selectionStyle               = UITableViewCellSelectionStyleNone;
        cell.Btype =index;
        _countText = [NSString stringWithFormat:@"%@条处理中",_dataSourceArr[index][@"count"]];
        if ([_countText  isEqual: @"0条处理中"]) {
            _countText = @"";
        }
        cell.Rlabel = _countText;
        return cell;
        ...
        cell.selectionStyle               = UITableViewCellSelectionStyleNone;
        cell.Btype =index;
        _countText = [NSString stringWithFormat:@"%@条已处理",_dataSourceArr[index][@"count"]];
        if ([_countText  isEqual: @"0条已处理"]) {
            _countText = @"";
        }
        cell.Rlabel = _countText;
        return cell;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    //    BlackListDetailViewController *VC = [[BlackListDetailViewController alloc]init];
    //    VC.type                           = _selectedIndex;
