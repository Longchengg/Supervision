//
//  ZhaobiaogonggaoViewController.m
//  Supervision
//
...
@interface ZhaobiaogonggaoViewController () <SYProgressWebViewDelegate>
...
@implementation ZhaobiaogonggaoViewController
...
{
    if (_webId == 0) {
        _webViewStr = @"https://xizang.zhaobiao.cn/";
    }else if(_webId == 1){
        _webViewStr = @"http://xizang.okcis.cn/";
    }else if(_webId == 2){
        _webViewStr = @"http://xzzbtb.gov.cn/";
    }else if(_webId == 3){
        _webViewStr = @"http://www.ccgp-xizang.gov.cn/";
    }else if(_webId == 4){
        _webViewStr = @"http://54.223.183.206:8098/protocol.html";
    }else{
        _webViewStr = @"https://saas.jz-rck.com/protocol.html";
    }
    [self webViewUIPush];
}
...
- (void)webViewUIPush
{
    ...
    [self.webView loadRequest:^(SYProgressWebView *webView, NSString *title, NSURL *url) {
        ...
        if (self.webId == 5 ||self.webId == 6) {
            weakWebView.title = @"用户协议";
        }else{
